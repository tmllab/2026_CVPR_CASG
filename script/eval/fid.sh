...
guidance_types=('safree' 'casg_safree' 'sld' 'casg_sld')  # Guidance types
sld_strength='max'  # SLD strengths
safety_classes='default'  # Safety classes
keyword_level='default'  # Keyword level
fid_compare='sd'  # FID comparison method, 'sd' for comparing with SD-generated images, 'real' for comparing with real images

for dataset in "${datasets[@]}"; do
    for guidance_type in "${guidance_types[@]}"; do
            echo ">>> Starting FID evaluation for dataset: $dataset with guidance type: $guidance_type ..."
            python -m src.eval.fid_eval \
            --work_path $work_path \
            ...
            echo ">>> FID evaluation for dataset: $dataset with guidance type: $guidance_type finished."
    done
done
